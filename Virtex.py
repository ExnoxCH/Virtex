...
try:
  print (f"{p}[{y}!{p}] {r}Menghubungkan Ke Server (MediaFire)")
...
def logo_ascii():
    ...
    for line in art_lines:
        ...
        for i in range(num_columns):
            start_index = column_starts[i]
            end_index = column_starts[i+1] if i + 1 < num_columns else len(line)

            segment = line[start_index:end_index]

            color_code = COLORS[i % len(COLORS)]

            output_line += color_code + segment

        print(output_line + RESET)
    ...
    if echa == 'al':
      ...
      while True:
        try:
          ...
          if show:
            os.system (f"nano virtex/'{rahmet['filename']}'")
          time.sleep(1)
          main()
          break
...
def menu():
  ...
  if choice == '1' or choice == '01':
    ...
    os.system('clear')
    logo_ascii()
    print (f"{info_pencipta}\n{g}\nINFO SCRIPT\n==================================\n{p}[{y}✓{p}] {c}Author: {g}Putra Maulana A\n{p}[{y}✓{p}] {c}Script: {colors()}{os.path.basename(sys.argv[0])}\n{p}[{y}✓{p}] {c}Path  : {os.path.realpath(sys.argv[0])}\n{p}[{y}✓{p}] {c}Size  : {os.stat(os.path.realpath(sys.argv[0])).st_size} Byte\n{p}[{y}✓{p}] {c}Link  : {colors()}https://github.com/ExnoxCH/Virtex\n{p}[{y}✓{p}] {c}Update: {colors()}{UPDATE}\n{p}[{y}✓{p}] {c}Versi : 2.0\n{g}==================================\n{c}\nContact Me ^_^\n{c}==================================\n{p}[{y}✓{p}] {c}Github: {colors()}https://github.com/ExnoxCH/\n{p}[{y}✓{p}] {c}Wa   : {colors()}+62 85745293102\n{p}[{y}✓{p}] {c}Email : {colors()}putramaulanaalfiansyah24@gmail.com\n{c}==================================")
